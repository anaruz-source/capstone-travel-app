...
const outerUserController = async (req, res) => {
        ...
        try {
            ...
            if (Object.keys(req.body).length == 2) { // email or username and password
                ...
                const user = await User.findOne({
                    [searchKey]: req.body.email
                }) // we want the value of searchKey as key not the literal searchKey, thus  [] used

                if (hashed === user.password) {

                    user.status = 'online'

                    placeHolders[user._id] = {
                        user,
                        connDate: new Date()
                    }
    ...
    profileController = (req, resp) => {
        ...
        resp.render(path.resolve(__dirname + '/../../../dist/templates/profile.html.twig'), {
            u: u.user,
